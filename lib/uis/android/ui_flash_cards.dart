...
class _UiFlashCardsState extends State<UiFlashCards>
  ...
  final CodeFolder codeFolder = CodeFolder.instance;
  ...
  Future<void> _loadFlashcards() async {
    ...
    try {
      final jsonStr = await codeFolder.getFlashcards(widget.filename!);
  ...
  Future<void> _saveFlashcards({bool isAutoSave = false}) async {
    ...
    try {
      await codeFolder.updateFlashcards(title, data);
      await codeFolder.refreshFiles();
      if (!mounted) return;
      if (!isAutoSave) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
